...
local function title(bufnr)
    ...
    elseif file == '' then
        ...
        -- return fn.pathshorten(fn.fnamemodify(file, ':p:~'))
        return fn.pathshorten(fn.fnamemodify(file, ':p:~:.'),9)
    end
end
...
local function devicon(bufnr, hl_base)
    ...
    local icon, devhl, hl_colour
    local devicons_ok, devicons = pcall(require, "nvim-web-devicons")
    if devicons_ok then
        if filetype == 'fugitive' then
            icon, devhl = devicons.get_icon('git')
            _, hl_colour = devicons.get_icon_colors('git')
        elseif filetype == 'vimwiki' then
            icon, devhl = devicons.get_icon('markdown')
            _, hl_colour = devicons.get_icon_colors('markdown')
        elseif buftype == 'terminal' then
            icon, devhl = devicons.get_icon('zsh')
            _, hl_colour = devicons.get_icon_colors('zsh')
        else
            ---@diagnostic disable-next-line:missing-parameter
            icon, devhl = devicons.get_icon(file, fn.expand('#' .. bufnr .. ':e'))
            _, hl_colour = devicons.get_icon_colors(file, fn.expand('#' .. bufnr .. ':e'))
        end
    end
...
local function cell(index, selected)
    ...
    local ret = string.format('%s %s.%s%s%s ',
        common,
    index,
        devicon(bufnr, hl),
        title(bufnr),
