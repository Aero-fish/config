...
def set_wallpaper():
    ...
    if sys.platform.startswith("win32"):
        logging.info("No code to set wallpaper.")
...
def download_wallpaper(num_of_new_wallpaper: int = 0):
...
def check_number_of_new_wallpapers() -> int:
    ...
    latest_bing_wallpaper_date, _ = get_latest_wallpaper_date_and_url()
    latest_bing_wallpaper_date = datetime.datetime.strptime(
        latest_bing_wallpaper_date, DATE_FMT
    ).date()
    return (latest_bing_wallpaper_date - last_download_date).days
    ...
    try:
        ...
        if has_internet:
            ...
            if num_of_new_wallpaper > 0:
                download_wallpaper(num_of_new_wallpaper=num_of_new_wallpaper)
